    ...
    try {
      ...
      try {
        ...
        if (uploadResponse.ok) {
          ...
          console.log('✅ Garment and person images uploaded to Cloudinary');
        } else {
          console.warn('⚠️ Image upload failed, using filenames');
        }
      } catch (uploadError) {
      ...
      try {
        ...
        if (combinedUploadResponse.ok) {
          ...
          console.warn('⚠️ Combined image upload failed, using base64');
        }
      } catch (combinedUploadError) {
        console.warn('⚠️ Combined image upload error:', combinedUploadError);
      }
      ...
      // Step 7: Save metadata to database with Cloudinary URLs
      setCurrentStep(7);
      ...
      try {
        ...
        const metadataResponse = await fetch('/api/save-tryon-metadata', {
          ...
          body: JSON.stringify({
            jobId,
            cloudinaryUrls: {
              output: data.image, // ✅ Original Cloudinary URL from virtual-tryon API
              garment: garmentCloudinaryUrl, // ✅ Cloudinary URL for garment
              person: personCloudinaryUrl, // ✅ Cloudinary URL for person
              combined: combinedCloudinaryUrl, // ✅ Cloudinary URL for combined image
            },
            model: data.model,
