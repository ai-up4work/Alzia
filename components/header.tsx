...
import { useCart as useRetailCart } from "@/lib/cart-context"
import { useCart as useWholesaleCart } from "@/lib/wholesale-cart-context"
import { useAuth } from "@/lib/auth-context"
import Image from "next/image"
import { getRoleBasedRedirect } from "@/lib/utils/role-redirect"

// ── Role-aware cart hook ──────────────────────────────────────────────────────
// Returns the correct cart (wholesale or retail) based on the user's role.
// This keeps both contexts completely isolated from each other.

function useRoleCart(role?: string) {
  const retailCart = useRetailCart()
  const wholesaleCart = useWholesaleCart()
  return role === "wholesaler" ? wholesaleCart : retailCart
}
...
export function Header() {
  ...
  // ✅ Picks wholesale or retail cart automatically based on role
  const { totalItems, openCart } = useRoleCart(user?.role)
  ...
  const renderUserAvatar = (size: "small" | "large") => {
    ...
    return (
      <div
        className={`${sizeClasses} rounded-full bg-gradient-to-br from-primary/20 to-secondary/20 border-2 border-primary/30 flex items-center justify-center hover:border-primary/50 transition-all`}
      >
  ...
  // Cart badge color differs by role so users can visually tell which cart is active
            ...
            <div className="flex items-center gap-2 md:gap-4">
              ...
              >
                ...
                {!showLoading && totalItems > 0 && (
                  <span
                    className={`absolute -top-1 -right-1 w-5 h-5 text-xs rounded-full flex items-center justify-center font-medium ${cartBadgeClass}`}
                  >
