...
export function ProductCard({
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    toast.success("Added to cart!", {
      ...
      action: { label: "View Cart", onClick: () => router.push("/account/cart") },
    })
  ...
  const handleWishlist = (e: React.MouseEvent) => {
    ...
    e.stopPropagation()
    toggleItem(product)
    toast.success(inWishlist ? "Removed from wishlist" : "Added to wishlist!", { description: product.name })
  }
          ...
          <div>
            ...
            <div className="flex items-center gap-1">
              <div className="flex gap-0.5">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} className={`w-4 h-4 ${i < Math.floor(product.rating_avg) ? "fill-accent text-accent" : "fill-muted text-muted"}`} />
                ))}
              </div>
          ...
          <div className="flex items-center justify-between mt-4">
            <div>
              ...
              {product.stock_quantity > 0 && product.stock_quantity <= 5 && (
                <p className="text-xs text-orange-600 font-medium mt-1">Only {product.stock_quantity} left</p>
              )}
            </div>
            <div className="flex gap-2">
              <Button size="sm" variant="outline" className="rounded-full bg-transparent" aria-label={inWishlist ? "Remove from wishlist" : "Add to wishlist"} onClick={handleWishlist}>
                <Heart className={`w-4 h-4 ${inWishlist ? "fill-red-500 text-red-500" : ""}`} />
              </Button>
              <ProductShareButton product={product} className="h-9 w-24 p-0 rounded-full" size="sm" variant="outline" />
              <Button size="sm" className="rounded-full" onClick={handleAddToCart} disabled={product.stock_quantity === 0 || isAddingToCart}>
                {isAddingToCart ? (
                  <><Check className="w-4 h-4 mr-2 animate-in zoom-in" />Added</>
                ) : (
  ...
  return (
    <div className="group bg-card rounded-2xl lg:rounded-3xl overflow-hidden border border-border/50 shadow-sm hover:shadow-lg transition-all duration-500">
      ...
      <Link href={`/product/${product.slug}`} className="relative block aspect-square overflow-hidden bg-muted">
        {/* ✅ FIX: was bare <img>, now uses Next.js <Image> for optimisation + consistency with list view */}
      ...
      <div className="p-4 lg:p-5">
        ...
        <div className="flex items-center gap-1 mb-3">
          <div className="flex gap-0.5">
            {[...Array(5)].map((_, i) => (
              <Star key={i} className={`w-3.5 h-3.5 ${i < Math.floor(product.rating_avg) ? "fill-accent text-accent" : "fill-muted text-muted"}`} />
            ))}
          </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-1.5">
            ...
            >
              {isAddingToCart ? <Check className="w-4 h-4 animate-in zoom-in" /> : <ShoppingBag className="w-4 h-4" />}
            </Button>
          </div>
        </div>
      </div>
